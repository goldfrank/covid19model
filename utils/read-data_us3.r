...
read_obs_data <- function(countries){
  ...
  d = read.csv('data/us/COVID-19-up-to-date_us.csv')
...
read_ifr_data <- function(){
  ifr.by.country <- read.csv("data/us/popt_ifr_us.csv")
...
read_interventions <- function(countries){
  interventions = read.csv('data/us/interventions_us.csv', stringsAsFactors = FALSE)
